  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed top-5 right-5 sm:right-8 md:right-12 z-50 inline-flex items-center justify-center gap-2 text-xs md:text-sm font-black uppercase tracking-wider px-4 sm:px-5 py-2 sm:py-2.5 rounded-full transition-all border ${
          isMenuOpen
            ? "bg-transparent md:bg-[#111318] text-white border-white/60 shadow-none hover:bg-white/10"
            : isHomePage
            ? "bg-transparent md:bg-[#0B0C10] text-white border-white/60 md:border-white/20 hover:bg-white/10 shadow-none md:shadow-black/40"
            : "bg-transparent md:bg-[#0B0C10] text-white md:text-white border-black/30 md:border-white/20 hover:bg-black/5 shadow-none md:shadow-black/30"
        }`}
      >
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-40 bg-[#0B0C10] flex flex-col justify-between pt-24 pb-8 transition-opacity duration-300">
          ...
          <div className="max-w-[1500px] w-full mx-auto px-6 md:px-12 flex items-center justify-between text-xs text-white/50 pt-4 border-t border-white/10">
            <span>SMIT KHATRI — FULL-STACK DEVELOPER</span>
            <span className="text-[#F14E08] font-bold md:hidden inline">SWIPE / TAP WHEEL TO NAVIGATE</span>
          </div>
        </div>
      )}
    </>
